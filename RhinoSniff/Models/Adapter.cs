    ...
    public static async Task InitAdapters()
    {
        ...
        catch (Exception ex)
        {
            if (ex.Message.ToLower().Contains("unable to load dll"))
            {
                ...
                MessageBox.Show(
                    $"{Resources.ADAPTER_EXCEPTION}\n\nYou will need to install Npcap for RhinoSniff to work\n\nNeed help installing Npcap? Follow the setup guide at https://rhinosniff.net/support/setup\n\nInstalled Npcap and RhinoSniff is still not working?\nReport this to us: failed to load dynamic link library. Npcap capture driver was not found or corrupt.",
