    ...
    public partial class ArpWindow : Window
    {
            ...
            ArpDevice arpDevices = new())
        {
            try
            {
                ...
                IsScanning = true;
                TitleBox.Text = $"RhinoSniff v{Assembly.GetExecutingAssembly().GetRhinoSniffVersion()} - ARP Poisoning";
                    ...
                    try
                    {
                        ...
                        if (args.Data.ToLower().Contains("unreachable"))
                        {
                            MessageBox.Show(
                                ...
                                "RhinoSniff", MessageBoxButton.OK, MessageBoxImage.Warning);
                            return;
                        }
        ...
        private async void ScanLocalDevices()
        {
            ...
            {
                await ex.AutoDumpExceptionAsync();
                MessageBox.Show("Failed to scan for devices", "RhinoSniff", MessageBoxButton.OK,
        ...
        private async void TogglePoisonCommandEvent(object sender, ExecutedRoutedEventArgs e)
        {
            try
            {
                ...
                if (Globals.Container.GetInstance<IThemeUtils>().MsgBox(new MsgBox
                {
                    ...
                    Message =
                        "ARP poisoning will intercept traffic from the target device through your PC. The target's internet connection will be maintained via packet forwarding. If Null-route is enabled, the target will lose internet access entirely. Continue?"
